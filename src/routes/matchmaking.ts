...
const useCustom = process.env.USE_CUSTOM_MATCHMAKING_CODE === 'true';
...
export default (app: Hono) => {
  ...
  app.get("/fortnite/api/matchmaking/session/:sessionId", (c) => {
    ...
    if (useCustom) {
      ...
      const serverAddress = process.env.GAMESERVER_IP!.split(":")[0] || "";
      const serverPort = Number(process.env.GAMESERVER_IP!.split(":")[1]);
